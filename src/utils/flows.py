...
class linIAF(nn.Module):
    ...
    def forward(self, l, z):
        ...
        if self.cuda:
            lt_mask = lt_mask.to(self.device)
            I = I.to(self.device)
...
class NormalizingFlows(nn.Module):
    ...
    def __init__(self, in_features, n_flows=1, h_last_dim=None, flow_type=PlanarNormalizingFlow, device='cuda'):
        ...
        for i, features in enumerate(reversed(in_features)):
            self.flows += [nn.ModuleList([flow_type(features).to(self.device) for _ in range(n_flows)])]
...
class HouseholderFlow(nn.Module):
    ...
    Presents a sequence of normalizing flows as a torch.nn.Module.
    """
    def __init__(self, in_features, auxiliary, n_flows=1, h_last_dim=None, flow_type=HFlow, flow_flavour="hf", device='cuda'):
        ...
        flows = []
        for i, features in enumerate(reversed(in_features)):
            flows += [flow_type().to(self.device)]
    ...
    def forward(self, z, h_last, auxiliary=False):
        ...
        j = 0
        if self.n_flows > 0:
            v['1'] = self.v_layers[0][0].to(self.device)(h_last)
            if not auxiliary:
            ...
            for j in range(1, self.n_flows):
                v[str(j + 1)] = self.v_layers[0][j].to(self.device)(v[str(j)])
                if not auxiliary:
...
class ccLinIAF(nn.Module):
    def __init__(self, in_features, n_flows=1, h_last_dim=None, flow_flavour="ccLinIAF", auxiliary=False, flow_type=linIAF):
        ...
        for i, features in enumerate(list(reversed(in_features))):
            flows += [flow_type(features).to(self.device)]
    ...
    def __init__(self, z_size, n_flows=2, num_hidden=0, h_size=50, forget_bias=1., conv3d=False, device='cuda'):
        ...
        for k in range(n_flows):
            ...
            if torch.cuda.is_available():
                z_feats = z_feats.to(self.device)
                zh_feats = zh_feats.to(self.device)
                linear_mean = linear_mean.to(self.device)
                linear_std = linear_std.to(self.device)
