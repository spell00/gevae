...
from src.utils.flows import Sylvester, TriangularSylvester, SylvesterFlows
...
class SylvesterVAE(Autoencoder):
    def __init__(self,
        ...
        try:
            self.flow_flavour = self.flow_type
        except:
            pass
        self.h_last = z_dim
        self.zs = torch.empty(size=(0, z_dim))
        self.zs_g = torch.empty(size=(0, z_dim))
        ...
        if self.flow_type == "o-sylvester":
            ...
            if self.flow_type == "h-sylvester":
                self.amor_q = nn.ModuleList([nn.Linear(self.h_last, self.n_flows * z_dim * self.num_elements) for z_dim in
                               self.z_dims])
            if self.auxiliary:
    ...
    def encode(self, x, y, a, auxiliary, i=0):
        ...
        if auxiliary:
            ...
            try:
               ...
               #      z1 = z
                #     mean_z, var_z = torch.mean(z1, 0).view(-1, 1), torch.var(z1, 0).view(-1, 1).to(
               #          self.device)
    ...
    def forward(self, x, y=torch.Tensor([]), a=torch.Tensor([]), k=0, auxiliary=False):
        ...
        self.zs = torch.cat((self.zs, z[-1].detach().cpu()))
        self.zs_g = torch.cat((self.zs_g, z[0].detach().cpu()))

        return x_mean, self.kl_divergence, z[-1]
